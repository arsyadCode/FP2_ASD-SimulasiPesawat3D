...
SDL_Window *CreateWindow(int width,int height,SDL_GLContext context) {
   ...
   //enable accelerated OpenGL rendering
    ...
    window = SDL_CreateWindow( "Tugas Besar", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN );
    if (window == NULL) {
...
void InitFloor(MESH *object) {
    VERTEX g_vertex_buffer_data[] = {
        //{{x,y,z},   {r,g,b,a}}
        /* TOP */
        {{ 60.0,  -2.0,  60.0},  {1, 0, 1, 1}, {0,1,0}}, //FRONT RIGHT
        {{ 60.0,  -2.0, -60.0},  {1, 0, 1, 1}, {0,1,0}}, //REAR RIGHT
        {{-60.0,  -2.0, -60.0},  {1, 0, 1, 1}, {0,1,0}}, //REAR LEFT
        {{-60.0,  -2.0,  60.0},  {1, 0, 1, 1}, {0,1,0}}, //FRONT LEFT
    ...
    //        penjelasan :               3 = (x,y,z)             10 = (xyz + rgba + xyz normal)   0 = attribute possition mulai dari 0
    glVertexAttribPointer( attribute_position, 3, GL_FLOAT, GL_FALSE, sizeof( float ) * 10, ( void * )0 );
    glVertexAttribPointer( attribute_color,  4, GL_FLOAT, GL_FALSE, sizeof( float ) * 10, ( void * )(3 * sizeof(float)));
    glVertexAttribPointer( attribute_normal,  3, GL_FLOAT, GL_FALSE, sizeof( float ) * 10, ( void * )(7 * sizeof(float)));
...
void InitMiniCube(MESH *object) {
    VERTEX g_vertex_buffer_data[] = {
        ...
        {{-0.25,  0.25,  0.25},  {1, 0, 1, 1}, {0,1,0}}, //FRONT LEFT

        /* BOTTOM */
        {{ 0.25, -0.25,  0.25},  {0, 0, 0, 1}, {0,1,0}}, //FRONT RIGHT
        {{ 0.25, -0.25, -0.25},  {0, 0, 0, 1}, {0,1,0}}, //REAR RIGHT
        {{-0.25, -0.25, -0.25},  {0, 0, 0, 1}, {0,1,0}}, //REAR LEFT
        {{-0.25, -0.25,  0.25},  {0, 0, 0, 1}, {0,1,0}}, //FRONT LEFT

        /* RIGHT WING BOTTOM */
        {{ 0.25, -0.25,  0.25},   {1, 0, 0, 1}, {0,1,0}}, //FRONT RIGHT
        {{ 0.25, -0.25, -0.25},   {1, 0, 0, 1}, {0,1,0}}, //REAR RIGHT

        /* LEFT WING BOTTOM */
        {{-0.25, -0.25, -0.25},   {1, 0, 0, 1}, {0,1,0}}, //REAR LEFT
        {{-0.25, -0.25,  0.25},   {1, 0, 0, 1}, {0,1,0}}, //FRONT LEFT
    ...
    //                                     3=(x,y,z)               10=(x,y,z+rgba+xn,yn,zn)     0=urutan mulai itu dari 0
    glVertexAttribPointer( attribute_position, 3, GL_FLOAT, GL_FALSE, sizeof( float ) * 10, ( void * )0 );
    glVertexAttribPointer( attribute_color,  4, GL_FLOAT, GL_FALSE, sizeof( float ) * 10, ( void * )(3 * sizeof(float)));
    glVertexAttribPointer( attribute_normal,  3, GL_FLOAT, GL_FALSE, sizeof( float ) * 10, ( void * )(7 * sizeof(float)));
...
int main(int argc, char * argv[]){
    ...
    int mixflag = 0;
	int mixresult = Mix_Init(mixflag); 
	if (mixresult != mixflag) {
		cout << "gagal set mixer utk pakai mp3\n";
	}
	else{
		cout << "selesai set mixer utk pakai mp3\n";

	}

    Mix_OpenAudio(44100, AUDIO_S16SYS, 2, 512);
	Mix_Music *my_music = Mix_LoadMUS("bossmusic.wav");
	Mix_Chunk *my_soundeffect = Mix_LoadWAV("blast.wav");
    
	if (my_music == NULL) {
		cout << "gagal load mp3\n";
	}
	else {
		cout << "selesai load mp3\n";
    ...
    MESH model_terrain;
    readfile_obj(&model_terrain,"landscape.obj");
    OBJECT terrain;
    mat4x4_identity(temp_matrix);
    initObject(&terrain,&model_terrain,temp_matrix);
    mat4x4_translate(terrain.model_matrix,0,-15,0);

    MESH model_baseland;
    InitFloor(&model_baseland);
    OBJECT base_land;
    mat4x4_identity(temp_matrix);
    initObject(&base_land,&model_baseland,temp_matrix);
    mat4x4_translate(terrain.model_matrix,0,0,30);

    MESH model_alien;
    readfile_obj(&model_alien,"AlienAnimal.obj"); //"Intergalactic_Spaceship.obj"
    OBJECT alien;
    mat4x4_identity(temp_matrix);
    initObject(&alien,&model_alien,temp_matrix);
    mat4x4_translate(alien.model_matrix,0,-2,20);
    mat4x4 alien_rot;
    mat4x4_rotate_Y(alien_rot,temp_matrix,180);
    mat4x4_dup(alien.model_matrix,alien_rot);
    ...
    while (running)
    {
            ...
            switch( event.type )
            {
                ...
                case SDL_KEYUP:
                    ...
                    if(  event.key.keysym.sym == SDLK_SPACE ) {
                        Mix_PlayChannelTimed(-1, my_soundeffect, 1, 1000);
        ...
        for (int i = 0; i <3; i++) {
            if (plane_pos[i] > 50000 ) {
                plane_pos[i] = 50000;
            }
            else if (plane_pos[i] < -50000 ) {
                plane_pos[i] = -50000;
            }
        ...
        vec3 atas={0,1,0};
        vec3 jarak={0,1,-20};
        vec3_dup(jarak,plane_front);
        jarak[1]=0;
        vec3_norm(jarak,jarak);
        vec3_scale(jarak,jarak,-20);
        vec3_add(cam_position,plane_pos,jarak);
        cam_position[1]=cam_position[1]+5;
        

        vec3_add(cam_target,cam_position,plane_front); // depan
        ...
        DrawObject( &alien,program01,pv_matrix,id);
        DrawObject( &pesawat,program01,pv_matrix,id);
        DrawObject( &terrain,program01,pv_matrix,id);
        DrawObject( &base_land,program01,pv_matrix,id);
        //==============================[ ASD ]===================================//
        ...
        while(kursorpeluru!=NULL){
            ...
            cout << "4"<<endl;
            vec4_scale(delta,peluru_front,(float)delta_frame_time * 100.0f / 1000);
            cout << "5"<<endl;
